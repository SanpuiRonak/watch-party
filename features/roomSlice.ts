...
export const roomSlice = createApi({
    ...
    endpoints: (builder) => ({
        getRooms: builder.query<Room[] | null, void>({
            queryFn: async () => {
                try {
                    const roomData = await getDataFromDB<Room[]>(DATABASE_KEYS.ROOM_DATA);
        ...
        saveRoom: builder.mutation<string, Room>({
            queryFn: async (room) => {
                try {
                    const existingRooms = await getDataFromDB<Room[]>(DATABASE_KEYS.ROOM_DATA) ?? [];
                    const updatedRooms = [...existingRooms, room];
                    const encryptData = await upsertDataInDB<Room[]>(updatedRooms, DATABASE_KEYS.ROOM_DATA);
