...
import { Maximize, Pause, PictureInPicture2, Play, Volume2, VolumeX } from "lucide-react";
...
export function VideoPlayer({ streamUrl, onVideoEvent }: VideoPlayerProps) {
    ...
    const canControl =
        ...
        room?.permissions.canSeek ||
        room?.permissions.canChangeSpeed;
    ...
    }, [serverVideoState?.playbackRate, serverVideoState]);
    ...
    useEffect(() => {
        ...
        if (serverVideoState.isPlaying && video.paused) {
            video.play().catch(error => logger.error("Video play error:", error));
        } else if (!serverVideoState.isPlaying && !video.paused) {
            video.pause();
        }
    }, [serverVideoState?.lastUpdated, serverVideoState]);
    ...
    const handlePlay = () => {
        logger.debug("[VideoPlayer] handlePlay called", { isOwner, canPlay: room?.permissions.canPlay });
        if (!canControl || (!isOwner && !room?.permissions.canPlay) || !videoRef.current) {
            logger.debug("[VideoPlayer] handlePlay blocked", {
                canControl,
                isOwner,
        ...
        logger.debug("[VideoPlayer] handlePlay proceeding - calling onVideoEvent");
    ...
    const handlePause = () => {
        logger.debug("[VideoPlayer] handlePause called", { isOwner, canPlay: room?.permissions.canPlay });
        if (!canControl || (!isOwner && !room?.permissions.canPlay) || !videoRef.current) {
            logger.debug("[VideoPlayer] handlePause blocked", {
                canControl,
                isOwner,
                canPlay: room?.permissions.canPlay,
                videoRef: !!videoRef.current,
        ...
        logger.debug("[VideoPlayer] handlePause proceeding - calling onVideoEvent");
        setIsUserAction(true);
    ...
    const handleSeeked = () => {
        if (
            !canControl ||
            (!isOwner && !room?.permissions.canSeek) ||
            !videoRef.current ||
            isUserAction
        )
            return;
    ...
    const togglePlayPause = () => {
        logger.debug("[VideoPlayer] togglePlayPause called", {
            canPlay: room?.permissions.canPlay,
            isOwner,
        });
        if (!videoRef.current || (!isOwner && !room?.permissions.canPlay)) {
            logger.debug("[VideoPlayer] togglePlayPause blocked - no permission or no video ref");
            return;
        }
                ...
                <div className="mb-4">
                    ...
                    >
                        <Slider
                            ...
                            step={1}
                            onValueChange={value =>
                                (isOwner || room?.permissions.canSeek) && handleSeek(value[0])
                            }
                            className="w-full"
                ...
                <div className="flex items-center justify-between text-white">
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            ...
                            title={
                                !isOwner && !room?.permissions.canPlay
                                    ? "Play/Pause disabled by owner"
                                    : ""
                            }
                        >
                            {isPlaying ? (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            <Button
                                ...
                                size="sm"
                                onClick={() =>
                                    (isOwner || room?.permissions.canChangeSpeed) &&
                                ...
                                title={
                                    !isOwner && !room?.permissions.canChangeSpeed
                                        ? "Playback speed control disabled by owner"
                                        : ""
                                }
                            >
                                {playbackRate}x
