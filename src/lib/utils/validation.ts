...
export const validateStreamUrl = async (url: string): Promise<{ isValid: boolean; finalUrl?: string }> => {
  try {
    ...
    if (BLACKLISTED_HOSTS.includes(parsedUrl.hostname.toLowerCase())) {
      console.error('Blacklisted hostname:', parsedUrl.hostname);
      return { isValid: false };
    }
    
    // Check for private IPs
    if (isPrivateIP(parsedUrl.hostname)) {
      console.error('Private IP address detected:', parsedUrl.hostname);
      return { isValid: false };
    }
    ...
    if (BLACKLISTED_HOSTS.includes(finalParsedUrl.hostname.toLowerCase()) || 
        isPrivateIP(finalParsedUrl.hostname)) {
      console.error('Final URL after redirect is blacklisted or private:', finalParsedUrl.hostname);
      return { isValid: false };
    }
