...
export function handleError(error: unknown, context?: string): NextResponse {
    ...
    if (error instanceof AppError) {
        logger.error(`${logContext} Operational error:`, {
        ...
        return NextResponse.json(
            buildErrorResponse(error.statusCode, message, error.isOperational, error.details),
    ...
    if (error instanceof Error) {
        ...
        if (error.name === "ValidationError") {
            return NextResponse.json(
                buildErrorResponse(ErrorCode.VALIDATION_ERROR, "Validation failed"),
        ...
        if (error.message.includes("not found")) {
            return NextResponse.json(
                buildErrorResponse(ErrorCode.NOT_FOUND, "Resource not found"),
                {
    ...
    logger.error(`${logContext} Unknown error type:`, error);
 ...
 */
export function throwValidationError(field: string, message: string, value?: unknown): never {
    throw new ValidationError(`Validation failed for field "${field}": ${message}`, {
        field,
        value,
    });
}
 ...
 */
export function assertValid(condition: boolean, field: string, message: string): asserts condition {
    if (!condition) {
