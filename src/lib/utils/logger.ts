 ...
 */
function sanitizeForLogging(obj: unknown, depth = 0, maxDepth = 5): unknown {
    if (depth > maxDepth) {
    ...
    for (const key in recordObj) {
        if (Object.prototype.hasOwnProperty.call(recordObj, key)) {
            ...
            if (/password|token|secret|key|auth/i.test(key)) {
                ...
                sanitized[key] = sanitizeForLogging(recordObj[key], depth + 1, maxDepth);
            }
        }
    }
...
class Logger {
     ...
     */
    userAction(action: string, userId: string, details?: unknown): void {
        if (!this.shouldLog(LogLevel.INFO)) return;
     ...
     */
    roomAction(action: string, roomId: string, userId?: string, details?: unknown): void {
        if (!this.shouldLog(LogLevel.INFO)) return;
