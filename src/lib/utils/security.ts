 ...
 */
export function validateString(value: unknown, fieldName: string, maxLength: number = 1000): string {
...
export function validateUserId(userId: unknown): string {
  ...
  if (!validUserIdRegex.test(sanitized)) {
    throw new Error('Invalid userId format - only alphanumeric characters, hyphens, and underscores allowed');
  }
 ...
 */
export function sanitizeTextInput(input: unknown, fieldName: string, maxLength: number = 1000): string {
