...
export function validateRoomNameLength(roomName: string): void {
    validateLength(roomName, "Room name", VALIDATION_LIMITS.MAX_ROOM_NAME_LENGTH);
}
...
export function validateUsernameLength(username: string): void {
    validateLength(username, "Username", VALIDATION_LIMITS.MAX_USERNAME_LENGTH);
}
...
export function validateStreamUrlLength(streamUrl: string): void {
    validateLength(streamUrl, "Stream URL", VALIDATION_LIMITS.MAX_STREAM_URL_LENGTH);
}
...
export function validateUserIdLength(userId: string): void {
    validateLength(userId, "User ID", VALIDATION_LIMITS.MAX_USER_ID_LENGTH);
}
...
export function validateRoomIdLength(roomId: string): void {
    validateLength(roomId, "Room ID", VALIDATION_LIMITS.MAX_ROOM_ID_LENGTH);
}
 ...
 */
export function validateObjectComplexity(obj: unknown, maxDepth = 10, currentDepth = 0): void {
    ...
    if (typeof obj === "object") {
        if (Array.isArray(obj)) {
            ...
            for (const key of keys) {
                validateObjectComplexity(recordObj[key], maxDepth, currentDepth + 1);
            }
        }
    }
}
 ...
 */
export function validateRoomCreationRequest(data: unknown): RoomCreationRequest {
    if (!data || typeof data !== "object") {
        throw new Error("Invalid request body");
    }

    const dataObj = data as Record<string, unknown>;
    const { roomName, streamUrl, ownerId, ownerName } = dataObj;
 ...
 */
export function validatePermissionsRequest(data: unknown): PermissionsUpdateRequest {
    if (!data || typeof data !== "object") {
        throw new Error("Invalid request body");
    }

    const dataObj = data as Record<string, unknown>;
    const { permissions, ownerId } = dataObj;
    ...
    const { canPlay, canSeek, canChangeSpeed } = permissionsObj;
