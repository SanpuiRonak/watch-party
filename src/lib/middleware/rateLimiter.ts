 ...
 */
export async function resetRateLimit(limiter: RateLimiterMemory, key: string): Promise<void> {
    try {
        await limiter.delete(key);
    } catch (error) {
        secureLogger.error("Error resetting rate limit:", error);
    }
}
    ...
    try {
        ...
        if (error instanceof Error && error.message === "Rate limit exceeded") {
            ...
            const retryAfter = info
                ? Math.ceil(info.msBeforeNext / 1000)
                : limiter.blockDuration || 60;
