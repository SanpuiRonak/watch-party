...
export default function CreateRoom() {
            ...
            if (response.ok) {
                ...
                if (user) {
                    ...
                    const existingRooms = JSON.parse(
                        localStorage.getItem(`myRooms_${user.id}`) || "[]",
                    );
                    const updatedRooms = [
                        roomData,
                        ...existingRooms.filter((r: { id: string }) => r.id !== data.roomId),
                    ];
                    localStorage.setItem(
                        `myRooms_${user.id}`,
                        JSON.stringify(updatedRooms.slice(0, 10)),
                    );
                }
    ...
    return (
        <AuthWrapper requireAuth redirectTo="/user">
